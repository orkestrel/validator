...
/**
 * Determine whether two values are deep clones (deeply equal with no shared references).
 *
 * @param a - First value (candidate clone)
 * @param b - Second value (original)
 * @param opts - Optional clone-check options
 * @returns `true` if `a` is a deep clone of `b`
 * @example
 * ```ts
 * const b = { x: [1] }
 * const a = JSON.parse(JSON.stringify(b))
 * isDeepClone(a, b) // true
 * ```
 */
...
/**
 * Internal deep compare implementation that returns either `{ equal: true }` or
 * a diagnostic describing the first difference found.
 *
 * @param a - First value to compare
 * @param b - Second value to compare
 * @param cfg - Internal configuration (identityMustDiffer, opts)
 * @returns Diagnostic result with equality boolean and difference metadata when unequal
 * @example
 * ```ts
